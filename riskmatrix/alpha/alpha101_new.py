...
def decay_linear(df, period):
    ...
    # print (f'decay linear: {df.shape} ==  == {df}')
    ...
    if len(df.shape) == 1:
        ...
        # print (f'na_lwma: {na_lwma.shape} = {type (na_lwma)}= {na_lwma}')
        # return na_lwma

        # Convert the NumPy array to DataFrame
        na_lwma_df = pd.DataFrame(na_lwma, index=df.index, columns=[df.name])
        # print(f'na_lwma: {na_lwma_df.shape} = {type(na_lwma_df)} = {na_lwma_df}')
        return na_lwma_df
    else:
...
def alpha45(close, volume):
    ...
    # alpha = rank(sma(delay(close, 5), 20)) * x
    # for_test = correlation(ts_sum(close, 5), ts_sum(close, 20), 5)
    # print (f'for test only === {for_test}')
    # alpha = rank(correlation(ts_sum(close, 5), ts_sum(close, 20), 5))
    # alpha = correlation(ts_sum(close, 5), ts_sum(close, 20), 2)
    # print (f'alpha45 after:\n {alpha}')
...
def compute_alpha101(
    ...
    input_quote_volume: pd.DataFrame = pd_result_hour[
        ["close_time", "symbol", "quote_volume"]
    ].pivot(index="close_time", columns="symbol", values="quote_volume")

    input_high: pd.DataFrame = pd_result_hour[["close_time", "symbol", "high"]].pivot(
        index="close_time", columns="symbol", values="high"
    )

    input_low: pd.DataFrame = pd_result_hour[["close_time", "symbol", "low"]].pivot(
        index="close_time", columns="symbol", values="low"
    )
    ...
    input_return: pd.DataFrame = pd_result_hour[
        ["close_time", "symbol", "returns"]
    ].pivot(index="close_time", columns="symbol", values="returns")

    input_vwap: pd.DataFrame = input_quote_volume.div(input_volume)
    ...
    required_columns = {}
    calc_factor_list = full_required_columns.keys()
    ...
    for func_name in required_columns:
        print (f'get alpah func {func_name}')
    ...
    for func_name, func in alpha_functions.items():
        ...
        if isinstance(result, pd.DataFrame):
            ...
            raise ValueError(f"Unexpected result type for {func_name}: {type(result)}")
